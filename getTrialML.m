function [goodtrials info] = getTrialML(params,taskTrials,evts4,evts6,evts9,evts7)
...
if size(evts9,1) - size(goodtrials,1) == 1
    ...
    StimOff = evts7(gtE6(gtE6(:,2)==1));
    FixOn = evts4(gtE6(gtE6(:,2)==1));
else
...
ftime = [ftime{:}];
a = [ftime.time]';
info(:,11) = floor((info(:,8) - a) * params.Fs); % Stim onset datapoint from fix onset
ctl = a(1);
a(1) = floor((ctl - taskTrials(goodtrials(1)).fixationOn) * params.Fs);
info(:,10) = floor((a - ctl) * params.Fs); % Fix onset location
info(1,10) = a(1);
info(:,12) = 1:size(info,1);
info(:,13) = StimOn; % Stim onset from Plexon evets6 for good trials
info(:,16) = StimOff;
info(:,17) = FixOn;
info(:,18) = StimOn - evts4(goodtrials(1));
